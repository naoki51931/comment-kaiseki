...
from app.services.search import documents_for_user, search_database, search_public_database
...
def test_japanese_comment_and_ai_comment_are_searchable_after_submission() -> None:
    ...
    with sessions() as db:
        ...
        game = Game(user_id=user.id, original_filename="search.kif", storage_path="/tmp/search.kif", file_size_bytes=1, source_encoding="utf-8", played_at=date(2026, 8, 13), user_side="SENTE", is_public=True, initial_sfen="startpos", usi_moves=["7g7f"], move_count=1, normalized_hash="s" * 64, analysis_status="COMMENT_REQUIRED")
        ...
        public_results = search_public_database(db, "詰んでた", 20)
        assert public_results[0]["source_type"] == "comment_answer"
        assert public_results[0]["title"] == "1手目・匿名コメント1"
        assert public_results[0]["move_count"] == 1
        assert search_public_database(db, "", 20, move_number=1)
        assert search_public_database(db, "", 20, move_number=30) == []

        game.is_public = False
        db.commit()
        assert search_public_database(db, "詰んでた", 20) == []
...
def test_unsubmitted_comment_is_not_searchable_or_indexed() -> None:
    ...
    with sessions() as db:
        ...
        db.add(user); db.flush()
        game = Game(user_id=user.id, original_filename="draft.kif", storage_path="/tmp/draft.kif", file_size_bytes=1, source_encoding="utf-8", played_at=date(2026, 8, 13), user_side="SENTE", is_public=True, initial_sfen="startpos", usi_moves=["7g7f"], move_count=1, normalized_hash="d" * 64, analysis_status="COMMENT_REQUIRED")
        db.add(game); db.flush()
        ...
        assert search_public_database(db, "秘密コメント", 20) == []
